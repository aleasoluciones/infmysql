        ...
        with context("unhappy path"):
            with context("when executing with an invalid column"):
                with it("raises exception from wrapped library"):
                    ...
                    expect(_execute_query_with_invalid_column).to(
                        raise_error(mysql_exceptions.OperationalError, 1054)
                    )
            ...
            with context("when executing with a malformed query"):
                with it("raises exception from wrapped library"):
                    ...
                    expect(_execute_query_with_a_malformed_query).to(
                        raise_error(mysql_exceptions.ProgrammingError, 1064)
                    )
                    ...
                    expect(result).to(equal((
                        {
                            ...
                            "size": 40,
                            "active": 0,
                            "date": datetime.fromtimestamp(7300),
                        },
                        {
                            "id": 2,
                            "item": "item_b",
                            "size": 20,
                            "active": 1,
                            "date": datetime.fromtimestamp(3700),
                        },
                    )))
